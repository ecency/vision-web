...
import { rawVestsToAsset, toReceivedVestingShares } from "../utils/received-vesting-shares";
...
const DELEGATIONS = {
  outgoing_delegations: [{ delegatee: "ecency", amount: "350000000000", operation_id: "1", block_num: 2 }],
  incoming_delegations: [
    { delegator: "bob", amount: "1000000", operation_id: "3", block_num: 4 },
    { delegator: "carol", amount: "903311000000", operation_id: "5", block_num: 6 },
    { delegator: "dave", amount: "12345678901234567", operation_id: "7", block_num: 8 },
  ],
};
...
describe("rawVestsToAsset", () => {
  it("renders raw vests as the legacy asset string without losing digits", () => {
    ...
    expect(rawVestsToAsset("12345678901234567")).toBe("12345678901.234567 VESTS");
  });
});
...
describe("toReceivedVestingShares", () => {
  it("takes the incoming half, largest first, in the legacy shape", () => {
    expect(toReceivedVestingShares("alice", DELEGATIONS)).toEqual([
      { delegatee: "alice", delegator: "dave", vesting_shares: "12345678901.234567 VESTS" },
      { delegatee: "alice", delegator: "carol", vesting_shares: "903311.000000 VESTS" },
      { delegatee: "alice", delegator: "bob", vesting_shares: "1.000000 VESTS" },
    ]);
    expect(toReceivedVestingShares("alice", null)).toEqual([]);
    expect(toReceivedVestingShares("alice", { outgoing_delegations: [], incoming_delegations: [] })).toEqual([]);
  });
});
...
describe("received-delegation queries", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ConfigManager.setQueryClient(new QueryClient({ defaultOptions: { queries: { retry: false } } }));
  });

  it.each([
    ["getReceivedVestingSharesQueryOptions", getReceivedVestingSharesQueryOptions, ["wallet", "received-vesting-shares", "alice"]],
    ["getHivePowerDelegatingsQueryOptions", getHivePowerDelegatingsQueryOptions, ["assets", "hive-power", "delegatings", "alice"]],
  ] as const)("%s reads balance-api, never the Ecency endpoint, under its own key", async (_name, build, key) => {
  ...
  it("shares one balance-api request between the totals and the list", async () => {
    ...
    await (getReceivedVestingSharesQueryOptions("alice").queryFn as () => Promise<unknown>)();
    await (getHivePowerDelegatingsQueryOptions("alice").queryFn as () => Promise<unknown>)();
