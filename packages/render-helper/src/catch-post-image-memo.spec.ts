...
// Entry memo slots hold { body, meta, value }; read the value.
const slot = (k: string): string | null | undefined => cacheGet<{ value: string | null } | undefined>(k)?.value
...
describe('catchPostImage memoizes null results', () => {
  it('stores a null answer, distinguishable from no answer at all', () => {
    ...
    expect(slot(key(e, 0, 0))).toBeUndefined()
    expect(catchPostImage(e, 0, 0, 'match')).toBeNull()
    expect(slot(key(e, 0, 0))).toBeNull()
    expect(catchPostImage(e, 0, 0, 'match')).toBeNull()
  })
  ...
  it('keys the memo on size and format, as before', () => {
    ...
    expect(slot(key(e, 0, 0))).toBeNull()
    expect(slot(key(e, 600, 500))).toBeUndefined()
    catchPostImage(e, 600, 500, 'match')
    expect(slot(key(e, 600, 500))).toBeNull()
  })
  ...
  it('keeps fast and full answers in separate slots', () => {
    ...
    expect(slot(key(e, 0, 0, true))).toBeNull()
