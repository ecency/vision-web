...
function isStandalone(text3, idx) {
...
function firstStandalone(text3, re) {
  for (const m of text3.matchAll(re)) {
    const idx = m.index ?? 0;
    if (isStandalone(text3, idx)) return { url: m[0], pos: idx };
  }
  return null;
}
...
function blankUnequalAnchors(cleaned) {
  return cleaned.replace(
    HTML_ANCHOR_RE,
    (whole, href, text3) => decodeEntities(text3.trim()) === decodeEntities(href.trim()) ? whole : " ".repeat(whole.length)
  );
}
