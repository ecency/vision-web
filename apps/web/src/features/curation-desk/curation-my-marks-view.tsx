...
function MarksList({ state }: { state: CurationMarkState }) {
  const { data, isLoading, isError } = useMyMarks(state);
  ...
  if (!data?.items.length) return <p className="p-6 text-sm text-gray-500 text-center">{i18next.t("curation-desk.marks-view.empty")}</p>;
