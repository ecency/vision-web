const CONFIG = {
  visionConfig: {
    ...
    visionFeatures: {
      ...
      newsletter: {
        // Kill switch for the digest subscribe controls (community pages, creator profiles,
        // settings, homepage form, first-publish prompt, end-of-post prompt). Whether the
        // feature actually shows is decided at request time from NEWSLETTER_API_URL +
        // NEWSLETTER_SERVICE_TOKEN (see server/newsletter-internal `newsletterFeatureEnabled`):
        // one image serves every region, so a build-time flag cannot express "only where the
