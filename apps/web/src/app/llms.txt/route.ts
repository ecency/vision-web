...
- [JSON](${base}/@username/permlink.json): the post as structured JSON in a small envelope (\`type\`, \`canonical_url\`, \`source\`, \`content\`). \`content\` is the post record, and \`content.json_metadata\` is always a JSON object (\`{}\` when the post's metadata is missing, invalid, not a JSON object or nested more than 64 levels deep), never a JSON string. Append \`.json\`.
