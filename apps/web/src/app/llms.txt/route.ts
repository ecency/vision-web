...
- [JSON](${base}/@username/permlink.json): the post as structured JSON in a small envelope (\`type\`, \`canonical_url\`, \`source\`, \`content\`). \`content\` is the post record, and \`content.json_metadata\` is always a JSON object (\`{}\` when the post's metadata is missing, invalid or not a JSON object), never a JSON string. Append \`.json\`.
...
- In the post envelope, \`canonical_url\` is that post's Ecency URL, and an author's own declared canonical, where they published one, is \`content.json_metadata.canonical_url\` and can point somewhere else. In the discussion envelope, \`canonical_url\` is the thread root's Ecency URL and every entry of the map carries its own \`json_metadata\`.
